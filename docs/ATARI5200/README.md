...
This also means that the OS built-in HSIO functionality presence depends on which particular OS ROMs you use/install (the HSIO used to be part of the built-in OS ROM), if you use a plain stock OS ROM, the HSIO can be used through the PBI BIOS (see below).

You should supply XL/XE OS ROM as games/ATARI800/boot0.rom, or load a corresponding file through OSD (this file should be 16K in size). **Note:** this ROM file also applies to the 400/800 machine type when the 16K option is selected for the 400/800 ROM type (the original 400/800 needs a special board for that, the 16K option enables an equivalent of such a board).
...
Then, you can provide the PBI BIOS file as games/ATARI800/boot3.rom (8K in size), here there is no OSD menu option as the only file that is usable for this is distributed with the core (check the releases folder).
...
There is also an OSD option to ignore bootX.rom files to prevent any confusion as to which file (bootX.rom or menu selected one) is loaded on core start.
...
### PokeyMax (Atari800)
One of the recent additions to the 800 core is the full integration of the PokeyMax implementation by Mark Watson. When disabled, the system behaves as if there are no extra Pokeys or sound devices, PokeyMax does not identify itself to the Atari, however, basic sound settings and mixing are still configurable. In general, the PokeyMax infrastrucutre is fully responsible for stereo (detection), channel selection and levels, mixing, etc. The top-level option from the main system additionally allows to define stereo mix (something that PokeyMax does not support). When enabled, all devices (4 Pokeys, 2 SIDs, 2 PSGs, Covox with a DMA sample engine) from the full PokeyMax config are available. Two feature details are different, there are only two output channels available (channels 3 and 4 for digital left and right, there are no analog channels 1 and 2, nor SPDIF channel 5 provided by PokeyMax itself relying on the one provided by the main system), according registers are ignored / fixed, and there is no Flash memory support (but the persistent PokeyMax options are configurable from the OSD menu). For compatibility with the older core and for software that does not allow COVOX address configuration, the writes to COVOX basic channels are mirrored at D600. Finally, the SID chip emulation in PokeyMax requires the SID specific data from the sid_data.bin file placed along other system ROM files in the core home directory, once there the file is loaded automatically upon core start.
...
Note also, that at least in this mode, the XEX loader is not really bulletproof when loading subsequent XEX files (due to the nature of OS-A/B and the construction of the XEX loader), a manual cold reset (F10) might be sometimes required in-between loading of two (different) XEX files in the classic 800 mode.
...
With the recent core releases (starting March 2026) the 800 core also supports writing to the Flash cartridges (AtariMax, SIC!, DCart, J(atari)Cart, MegaCart 4MB), the changes are either saved automatically (look in the OSD for the option), or the save has to be triggered manually. In both cases, the original CAR/ROM/BIN file is overwritten with the changes made to the cart Flash memory by the running Atari software. The active option to mount images read-only prevents and cartridge image file updates.
